...
import { deactivateItem } from "../../shared/api";
...
import ItemDeletion from "../ModalViews/ItemDeletion";
...
const Graph = () => {
  ...
  const { loading, showDeleteModal, setShowDeleteModal, handleDelete } =
    useDeleteItem();
  ...
  useEffect(() => {
    // Reset the graph data when the centralNodeId changes or the user changes the skip/limit
    console.log("central node or skip/limit changed");
    setData({
      nodes: [
        ...Object.values(state.data.nodes).map((n) => ({ ...n, id: n._id })),
      ], // TODO: Make _id "id" in API
  ...
  }, [state.centralNodeId, state.page, state.settings.graph.limit]);

  useEffect(() => {
    setData((prevData) => {
      let newGraphNodes = prevData.nodes.map((n) => {
        // get current node from state
        let currentNode = state.data.nodes[n._id];

        let newColor;
        // check if the type property exists on the currentNode
        if (currentNode && currentNode.type) {
          // if type exists, then check if corresponding ontologyId2Detail exists
          let typeDetail = state.ontologyId2Detail?.nodes[currentNode.type];
          // if detail exists, get color. Otherwise, fallback to n.color
          newColor = typeDetail?.color ?? n.color;
        } else {
          // if type doesn't exist, fallback to n.color
          newColor = n.color;
        }
      ...
      let newGraphLinks = [
        ...prevData.links.map((l) => {
          // get current link from state
          let currentLink = state.data.links[l._id];

          let newColor;
          // check if the type property exists on the currentLink
          if (currentLink && currentLink.type) {
            // if type exists, then check if corresponding ontologyId2Detail exists
            let typeDetail = state.ontologyId2Detail?.edges[currentLink.type];
            // if detail exists, get color. Otherwise, fallback to l.color
            newColor = typeDetail?.color ?? l.color;
          } else {
            // if type doesn't exist, fallback to l.color
            newColor = l.color;
          }
  ...
  const handleReviewItem = async (isNode, item) => {
    await handleReview({ isNode: isNode, itemId: item._id, reviewAll: false });
  };

  const handleItemActivation = async (isNode, item) => {
    // Toggle deletion modal
    // console.log("Delete item(s)");
      ...
      <>
        {/* {showDeleteModal && (
          <ItemDeletion
            ...
            handleClose={() => setShowDeleteModal({ open: false, data: null })}
        ...
        <Grid
          ...
          sx={{
            ...
            backgroundImage: `
            radial-gradient(lightgrey 1px, transparent 1px),
            radial-gradient(lightgrey 1px, transparent 1px)
        `,
        ...
        >
          <ForceGraph2D
            ...
            onNodeClick={(n, event) =>
              event.shiftKey
                ? handleReviewItem(true, n)
                : handleSetFocus(true, n)
            }
            ...
            nodeCanvasObject={(node, ctx, globalScale) => {
              ...
              const fontSize =
                (14 * Math.cbrt(node.value)) / globalScale ** 0.6;
              ...
              ctx.fillStyle = lighten(node.color, 0.5);
              ctx.fill();
            ...
            linkCanvasObject={(link, ctx, globalScale) => {
              ...
              if (hasErrors || hasSuggestions) {
                ...
                const flip =
                  angle > Math.PI / 2 || angle < -Math.PI / 2 ? -1 : 1;
